  ...
  module EmrEtlRunner
    class EmrJob
      ...
      def initialize(debug, enrich, shred, s3distcp, config, enrichments_array, resolver)
        ...
        @jobflow.instance_variable_set(:@region, config[:aws][:emr][:region]) # Workaround until https://github.com/snowplow/snowplow/issues/753
        ...
        if enrich
          ...
          to_hdfs = (self.class.is_cloudfront_log(config[:collectors][:format]) and s3distcp)
